...
from core.dbt.openlineage.common.facets import ParentRunMetadata, get_parent_run_facet, _get_job_type, \
    DbtOpenlineageJobType, get_job_name, OPENLINEAGE_DBT_JOB_NAME_ENV_VAR, OPENLINEAGE_PARENT_ID_ENV_VAR
from tests.openlineage_utils import dict_to_event_msg

DUMMY_UUID = "e3d27061-5519-4529-b357-93cd3b452245"

@pytest.mark.parametrize("env_var, expected_parent_run_metadata_data",
                        [
                            (
...
@pytest.mark.parametrize("event, expected_job_type",
                         [
                             (
                                     {
                                         "info": {
                             ...
                             (
                                     {
                                         "info":{
                                             "name": SQLQuery.__name__
                                         }
                                     },
                                     DbtOpenlineageJobType.sql.value
                             ),
                             (
                                     {
                                         "data":{
...
@pytest.mark.parametrize("env_var, event, expected_job_name",
                         [
                             (
                                "my-dbt-job",
                                {
