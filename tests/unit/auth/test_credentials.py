...
class TestPlatformCredentialFromToken:
    def test_service_token_classification(self):
        cred = PlatformCredential.from_token("dbtc_abc123", "ab123.us1.dbt.com", 42)
    ...
    def test_pat_classification(self):
        cred = PlatformCredential.from_token("dbtu_user_token", "ab123.us1.dbt.com", 7)
        assert isinstance(cred, PlatformCredential)
    ...
    def test_dbtu_prefix_only(self):
        cred = PlatformCredential.from_token("dbtu_", "host", 1)
        assert isinstance(cred, PlatformCredential)

    def test_similar_prefix(self):
        cred = PlatformCredential.from_token("dbtuser_abc", "host", 1)
        assert isinstance(cred, PlatformCredential)
