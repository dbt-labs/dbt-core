...
def dict_to_event_msg(e: Dict):
    """
    dict to event_msg instance
    """
    ...
    def assert_ol_event_match_rec(expected_event, actual_event, path_to_key):
        ...
        if isinstance(expected_event, dict):
            for key in expected_event:
                if key not in actual_event:
                    ...
                    assert_ol_event_match_rec(expected_event[key], actual_event[key], path_to_key + [key])
        # list case
        elif isinstance(expected_event, list):
            if len(expected_event) != len(actual_event):
                path_to_key_str = ".".join(path_to_key + ["[*]"])
                raise AssertionError(f"List length mismatch at path {path_to_key_str}, expected {len(expected_event)}, actual {len(actual_event)}")
            else:
                for i, (expected_item, actual_item) in enumerate(zip(expected_event, actual_event)):
        ...
        # literal string case with regex
            ...
            if not _is_match(expected_event, actual_event):
                ...
                assert expected_event == actual_event, f"Value mismatch at path=\"{path_to_key}\"\n It doesn't follow expected regex. Checkout value at path in actual event."
        # other literal cases
        else:
            path_to_key = ".".join(path_to_key)
            assert expected_event == actual_event, f"Value mismatch at path=\"{path_to_key}\", Checkout value at path in actual event."
...
def openlineage_handler_with_dummy_emit(monkeypatch):
    ...
    monkeypatch.setattr("dbt.openlineage.handler.OpenLineageHandler.emit", dummy_ol_handler.dummy_emit)
...
def openlineage_handler_with_raise_exception(monkeypatch):
    ...
    def raise_exception(self, exception: Exception):
        raise exception
    monkeypatch.setattr("dbt.openlineage.handler.OpenLineageHandler._handle_exception", raise_exception)
