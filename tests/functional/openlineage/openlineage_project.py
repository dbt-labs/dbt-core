...
class TestOpenLineage(OpenLineageJaffleShopProject):
    @pytest.mark.parametrize("dbt_command, expected_ol_events_path, expect_pass",
                            [
                                (
                                    ["run", "-s", "orders"],
                                    "./data/postgres/results/dbt_run_orders.json",
                                    True
                                ),
                                (
                                    ["test", "-s", "orders"],
                                    "./data/postgres/results/dbt_test_orders.json",
                                    True
                                ),
                                (
                                ...
                                (
                                    ["build", "-s", "orders"],
                                    "./data/postgres/results/dbt_build_orders.json",
                                    True
                                ),
    ...
    def test_openlineage_events(self, dbt_command, expected_ol_events_path, expect_pass, build_jaffle_shop_project,
                                openlineage_handler_with_dummy_emit, openlineage_handler_with_raise_exception):
        ...
        actual_ol_events = [ol_event_to_dict(e) for e in openlineage_handler_with_dummy_emit.emitted_events if e is not None]

        # todo remove the file written
        current_path = os.path.join(os.path.dirname(__file__), "actual_ol_events.json")
        with open(current_path, "w") as f:
            json.dump(actual_ol_events, f, indent=4)
    ...
    def test_openlineage_doesnt_make_dbt_fail(self, build_jaffle_shop_project, monkeypatch):
        ...
        monkeypatch.setattr("dbt.openlineage.handler.OpenLineageHandler.handle_unsafe", raise_exception)
        monkeypatch.setattr("dbt.openlineage.handler.OpenLineageHandler._handle_exception", dummy_handle_exception)
