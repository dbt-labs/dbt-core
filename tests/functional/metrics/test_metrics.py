...
class TestInvalidRefMetric:
    @pytest.fixture(scope="class")
    def models(self):
        return {
            "people_metrics.yml": invalid_models__people_metrics_yml,
            "people.sql": invalid_models__people_sql,
        }

    # tests that we get a ParsingException with an invalid model ref
    def test_simple_metric(
        self,
        project,
    ):
