...
class TestSourceNodeWithEnvVarInExternalLocation(BaseTestStateSelectionEnvVarConfig):
    @pytest.fixture(scope="class")
    def project_config_update(self):
        return {
            "flags": {
    ...
    def models(self):
        return {
            "schema.yml": schema_source_with_env_var_as_external_location_yml,
        }
    ...
    def test_change_env_var(self, project):
        ...
        manifest_json = get_artifact(project.project_root, "target", "manifest.json")
        assert (
            manifest_json["sources"]["source.test.jaffle_shop.customers"]["external"]["location"]
            == "@table/my_data/"
        )
        assert (
            manifest_json["sources"]["source.test.jaffle_shop.customers"]["external"][
                "unrendered_location"
            ]
            == "@{{ env_var('DBT_TEST_STATE_MODIFIED') }}/my_data/"
        )
        ...
        # Confirm env change changed location, but not unrendered_location
        manifest_json = get_artifact(project.project_root, "target", "manifest.json")
        assert (
            manifest_json["sources"]["source.test.jaffle_shop.customers"]["external"]["location"]
            == "@view/my_data/"
        ...
        # Assert no false negative after actual change to external.location
        write_file(
            schema_source_with_updated_env_var_as_external_location_yml,
            project.project_root,
            "models",
...
class TestSourceNodeWithJinjaInExternalLocation(BaseModifiedState):
    @pytest.fixture(scope="class")
    def project_config_update(self):
        return {
            "flags": {
    ...
    def models(self):
        return {
            "schema.yml": schema_source_with_jinja_as_external_location_yml,
        }

    def test_change_jinja_if(self, project):
        run_dbt(["run"])
        self.copy_state()
        # external.location is '@bar_stage/my_data/' when execute = False
        manifest_json = get_artifact(project.project_root, "target", "manifest.json")
        assert (
            manifest_json["sources"]["source.test.jaffle_shop.customers"]["external"]["location"]
            == "@bar_stage/my_data/"
        ...
        assert (
            ...
            == "@{{ ('foo' if execute else 'bar') }}_stage/my_data/"
        )
