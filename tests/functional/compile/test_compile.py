...
class TestUnknownMacroDemo:
    ...
    def test_compile_with_unknown_macro__raises_an_error(self, project, mocker) -> None:
        with pytest.raises(DbtException, match=".*callings a macro that does not exist.*"):
            run_dbt(["-d", "compile"])
