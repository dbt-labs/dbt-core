...
from dbt.events.types import OpenLineageException, CommandCompleted, FoundStats, NodeStart, NodeFinished, CatchableExceptionOnRun
...
from core.dbt.artifacts.schemas.results import NodeStatus
from core.dbt.openlineage.common.dataset_facets import extract_namespace, get_model_inputs, node_to_dataset, get_test_column
from core.dbt.openlineage.common.facets import ParentRunMetadata, get_parent_run_facet, get_dbt_version_facet, \
    get_job_type_facet, get_job_name, get_job_namespace, get_error_message_run_facet, get_dbt_command_parent_run_facet, \
    get_sql_job_facet
from core.dbt.openlineage.common.utils import get_attribute, get_event_time, generate_run_event
...
class OpenLineageHandler:
    ...
    def __init__(self, ctx: Context):
        ...
        self.dbt_command_metadata: Optional[ParentRunMetadata] = None
    ...
    def handle_unsafe(self, e: EventMsg):
        ...
        if event_name == FoundStats.__name__:
            openlineage_event = self._parse_dbt_start_command_event(e, self.ctx)
            self._setup_dbt_command_metadata(openlineage_event, self.ctx)

        elif event_name == CommandCompleted.__name__:
            openlineage_event = self._parse_command_completed_event(e, self.ctx)

        node_unique_id = get_attribute(e, "data.node_info.unique_id")

        if node_unique_id:
            if event_name == NodeStart.__name__:
                openlineage_event = self._parse_node_start_event(e, self.ctx)
            elif event_name == SQLQuery.__name__:
                openlineage_event = self._parse_sql_query_event(e, self.ctx)
            elif event_name in (SQLQueryStatus.__name__, CatchableExceptionOnRun.__name__):
                openlineage_event = self._parse_sql_query_status_event(e, self.ctx)
            elif event_name == NodeFinished.__name__:
                openlineage_event = self.parse_node_finished_event(e, self.ctx)
    ...
    def _parse_dbt_start_command_event(self, e: EventMsg, ctx: Context) -> RunEvent:
    ...
    def _parse_command_completed_event(self, e: EventMsg, ctx: Context) -> RunEvent:
    ...
    def _parse_node_start_event(self, e: EventMsg, ctx: Context) -> RunEvent:
        ...
        inputs = [node_to_dataset(input, dataset_namespace) for input in get_model_inputs(node_unique_id, self._manifest)]
        outputs = [node_to_dataset(self._manifest.nodes[node_unique_id], dataset_namespace)]
    ...
    def parse_node_finished_event(self, e: EventMsg, ctx: Context) -> RunEvent:
        ...
        if resource_type == NodeType.Test:
            ...
            dataset_namespace = extract_namespace(self._adapter_credentials)
            inputs = [self._get_test_input_dataset(node_unique_id, dataset_namespace, success)]
    ...
    def _parse_sql_query_event(self, e: EventMsg, ctx: Context) -> RunEvent:
        node_unique_id = e.data.node_info.unique_id
        sql_ol_run_id = str(generate_new_uuid())
        sql_start_at = get_event_time(e.info.ts.seconds)

        parent_run_facet = self._get_dbt_node_parent_run_facet(e)
        dbt_version_facet = get_dbt_version_facet()
        run_facets = {**parent_run_facet, **dbt_version_facet}
    ...
    def _parse_sql_query_status_event(self, e: EventMsg, ctx: Context) -> RunEvent:
        """
        ...
        event_time = get_event_time(e.info.ts.seconds)
        run_facets = dict(sql_ol_run_event.run.facets)
    ...
    def _get_sql_job_name(self, e: EventMsg) -> str:
        """
        The name of the sql job is as follows
        {node_job_name}.sql.{incremental_id}
        """
        #timestamp_str = get_event_time(e.info.ts.seconds)
        node_unique_id = e.data.node_info.unique_id
    ...
    def _get_data_quality_assertion_facet(self, test_node_id: str, success: bool) -> data_quality_assertions_dataset.Assertion:
        """
        {
    ...
    def _get_test_input_dataset(self, test_node_id: str, dataset_namespace: str, success: bool) -> Optional[Dataset]:
        test_node = self._manifest.nodes[test_node_id]
        attached_node_id = test_node.attached_node
        if attached_node_id:
            assertion_facet = self._get_data_quality_assertion_facet(test_node_id, success)
            attached_node = self._manifest.nodes.get(attached_node_id) or self._manifest.sources.get(attached_node_id)
            attached_dataset_name = ".".join(attached_node.fqn)
            return Dataset(
                namespace=dataset_namespace,
                name=attached_dataset_name,
                facets=assertion_facet,
            )
        else:
            return None
    ...
    def _get_dbt_node_parent_run_facet(self, e: EventMsg) -> Dict:
        node_unique_id = e.data.node_info.unique_id
