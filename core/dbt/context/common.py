...
class DatabaseWrapper(object):
    """
    Wrapper for runtime database interaction. Sets the current connection name.
    """
    def __init__(self, adapter):
...
def generate(model, config, manifest, source_config=None, provider=None):
    ...
    """
    # TODO: move this up the stack further!
    with get_adapter(config).connection_named(model.get('name')):
