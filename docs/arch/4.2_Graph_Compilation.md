...
Graph compilation transforms the parsed Manifest into an executable dependency graph. This happens in `Compiler.compile()` (`core/dbt/compilation.py`) and produces a `Graph` object wrapping a NetworkX DiGraph.
