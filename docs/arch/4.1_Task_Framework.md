...
Tasks are organized in a class hierarchy that adds capabilities at each level. The base classes live in `core/dbt/task/base.py` and `core/dbt/task/runnable.py`.
