...
impl MetadataAdapter for ExasolMetadataAdapter {
    ...
    fn list_relations_schemas_inner(
        ...
        // empty results, so no string-based type parsing is needed. Names stay
        // unquoted so Exasol's case-insensitive (uppercasing) resolution applies,
        // matching how relations render in materializations. The HashMap key must
        // match `relation.semantic_fqn()`, so both forms are carried as a tuple.
