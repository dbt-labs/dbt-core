...
export function devSite(): Plugin {
  ...
  // Version-scoped, because the artifacts are: a site generated by a dbt whose
  // information schema has moved on lives in a different directory.
  const DATA_DIR = 'info_schema/v1';
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use(`/${DATA_DIR}`, (req, res, next) => {
        ...
        // `views.sql` is served alongside the parquet — it is the view surface the
        // app executes, so without it there is no data layer at all.
        if (!/^[\w.]+\.(parquet|sql)$/.test(name)) return next();
        res.setHeader(
          'Content-Type',
          name.endsWith('.sql') ? 'text/plain' : 'application/octet-stream',
        );
