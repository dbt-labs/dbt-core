...
///
/// The installed `SKILL.md` carries dbt's injected `metadata` and so never
/// matches its source byte-for-byte; it is compared against regenerated bytes
/// instead. Every other file must still match exactly.
