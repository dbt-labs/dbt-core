...
mod tests {
    ...
    fn deserialize_serialize_is_unchanged() {
        let objects: Result<Vec<(serde_json::Value, serde_json::Value)>, serde_json::Error> = deserialize_serialize_loop(&get_input())
            .and_then(|v| {
                v.into_iter().map(|(s0, s1)| {
                    serde_json::from_str::<serde_json::Value>(&s0)
                        .and_then(|s0v| {
                            serde_json::from_str::<serde_json::Value>(&s1)
                                .map(|s1v| (s0v, s1v))
                        })
                }).collect()
        ...
        match objects {
            ...
            Ok(v) => {
                for pair in v {
                    match pair {
                        (serde_json::Value::Object(original), serde_json::Value::Object(looped)) => {
                            ...
                            for (key, value) in original.clone() {
                                ...
                                assert_eq!(looped_val, Some(&value), "original key value ({}, {}) expected in re-serialized result", key, value)
                            }
                            for (key, value) in looped.clone() {
                                let original_val = original.get(&key);
                                assert_eq!(original_val, Some(&value), "looped key value ({}, {}) not found in original result", key, value)
                            }
                        },
